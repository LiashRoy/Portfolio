        ...
        if (step.type === 'PLANNER_RESPONSE') {
            ...
            for (let tc of toolCalls) {
                if (tc.name === 'write_to_file' && tc.args.TargetFile === targetFile) {
                    ...
                    if (content.includes('async function loadGithubRepos')) {
                        console.log('loadGithubRepos is present at step', step.step_index);
                        fs.writeFileSync('index_step_' + step.step_index + '.html', content);
                    }
                }
            }
        }
